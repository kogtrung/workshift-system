...
import { lockShift } from '../../services/shifts/shiftLockApi'
...
export function ShiftLockModal({ open, onClose, groupId, shift, onLocked }) {
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      setError(err?.message || 'Không thể khóa ca')
    } finally {
      setLoading(false)
    }
  }
    ...
    <Modal open={open} onClose={onClose} maxWidthClass="max-w-md">
      ...
      <div className="p-6 space-y-4">
        ...
        <div className="flex justify-end gap-3 pt-2">
          <button type="button" onClick={onClose} disabled={loading} className="px-5 py-2.5 text-on-surface-variant font-semibold rounded-lg">
            Hủy
          </button>
          <button
            type="button"
